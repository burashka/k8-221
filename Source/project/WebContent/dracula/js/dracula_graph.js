...
Graph.Renderer.Raphael.prototype = {
	translate : function(point) {
		return [
				Math.round((point[0] - this.graph.layoutMinX) * this.factorX
						+ this.radius),
				Math.round((point[1] - this.graph.layoutMinY) * this.factorY
						+ this.radius) ];
	},
	...
	drawNode : function(node) {
		...
		if (node.shape) {
			...
			var opoint = [ oBBox.x + Math.round(oBBox.width / 2),
		...
		if (node.render) {
			...
			shape = this.r.set().push(
					...
					.push(
							this.r.text(point[0], point[1] + 30, node.label
									|| node.id));
		}
		shape.attr( {
			"fill-opacity" : .6
...
//версия из инета(вроде как должна лучше работать)
 ...
 *      paper.circle(100,100,10).tooltip(paper.rect(0,0,20,30));
 */
Raphael.el.tooltip = function(tp, tx, ty) {
	this.tp = tp;
	this.tp.hide();
	this.hover(function(event) {
		r = event.currentTarget.raphael;
		nx = r.getBBox().x;
		ny = r.getBBox().y;
		w = r.getBBox().width;
		h = r.getBBox().height;
...
Raphael.el.tooltip = function(tp, text, tx, ty) {
	this.tp = tp;
	this.text = text;
	this.hh = tp.getBBox().height / 2;
	this.tp.hide();
	this.text.hide();	
	this.hover(function(event) {
		r = event.currentTarget.raphael;
